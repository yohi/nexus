...
2. [ ] **依存関係の解決**: `npm install` を実行。
3. [ ] [AGENTS.md](AGENTS.md) **の確認**: セットアッププロトコルを事前に確認。
4. [ ] **ブートストラップ実行**: `node scripts/bootstrap.mjs` を実行し、必要なバイナリや設定を初期化。
5. [ ] **動作確認**: `npm test` を実行し、すべてのテストがパスすることを確認。

上記の手順は `node scripts/bootstrap.mjs` を実行することで一括実行することも可能です。
