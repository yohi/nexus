...
managed HTTP サーバーは、アクティブなセッション数が 0 になった時点で runtime を閉じ、`endpoint.json` とプロセスロックの両方を削除して終了します（`--idle-shutdown-ms` / `NEXUS_IDLE_SHUTDOWN_MS` で遅延を調整可能、デフォルト `0`）。起動後 30 秒以内に 1 つもクライアントが接続しなかった場合も、同様に自動終了します（起動 grace period）。コネクター自身も、descriptor が既定のタイムアウト（30 秒）以内に健全な状態で公開されない場合は、原因を標準エラー出力に書き込んで異常終了します。
...
- 起動時、`storage.rootDir` 直下に `nexus.pid` ファイルを作成し、排他的なアクセス権を獲得します。
