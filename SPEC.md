...
stdio 接続のみに対応した MCP クライアント（OpenCode など）から、既に起動している Nexus HTTP サーバーに接続するには、`nexus http-bridge` サブコマンドを使います。Bridge は独立したローカルプロセスとして起動し、標準入出力の JSON-RPC を Nexus の Streamable HTTP エンドポイントに転送します。同一プロジェクトに対しては常に 1 つの Nexus HTTP サーバーを常駐させ、各クライアントは Bridge を通じて共有します。

```text
┌─────────────────┐      ┌──────────────────┐      ┌─────────────────┐
│   MCP Client    │stdio │   nexus          │HTTP    │   nexus         │
│  (OpenCode etc.)│─────>│  http-bridge     │───────>│  --port 3001   │
└─────────────────┘      └──────────────────┘      └─────────────────┘
```

Bridge は Nexus サーバー自体を起動せず、あくまで stdio と HTTP 間のトランスポート中継のみを行います。デフォルトエンドポイントは `http://127.0.0.1:3001` で、`--url` 引数または `NEXUS_BRIDGE_URL` 環境変数で上書きできます。
