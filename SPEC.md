...
### 7.3. Single-Instance Process Lock (プロセスロック)
- 同一の `storage.rootDir` に対して複数プロセスが同時に起動し、データベースの破損やファイル監視の競合を引き起こすのを防ぐため、PIDファイルによるロック制御を行います。
- 起動時、`storage.rootDir` 直下に `nexus.pid` を原子性を担保した `O_EXCL`（`wx` フラグ）で作成し、自身のプロセスIDを書き込みます。
- ロックが既に存在する場合、記録されているPIDが生存しているか `process.kill(pid, 0)` を用いて確認します。死んでいる場合（クラッシュなどによる残存）は古いロックを削除して再取得を試み、生きている場合は起動を却下して終了します。
