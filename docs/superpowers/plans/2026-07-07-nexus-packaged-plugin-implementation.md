...
**Goal:** パッケージ版プラグイン（`yohi-nexus`）向けに、AWS Bedrock 埋め込みプロバイダと単一フラグ `NEXUS_PACKAGE_MODE`（埋め込み provider のハードロック）をコアに追加し、ソースミラー配布時に plugin.json を stage 時変換する。

**Architecture:** 設定駆動の単一コードベース。Bedrock は正規プロバイダとしてコアに追加（原本からも利用可）。`NEXUS_PACKAGE_MODE` は埋め込みを `provider=bedrock` に固定するハードロックを担う（`model=amazon.titan-embed-text-v2:0` / `dimensions=1024` / `region` は Spec §5.3 の deploy 可変値でロックしない）。メトリクス/TUI には触れない。配布は既存のソースミラー方式に「stage 時 plugin.json 変換」を足すだけ（`packages/dashboard` は同梱維持・ビルド無改修）。
...
- 埋め込みハードロック（パッケージ版）: `provider=bedrock` のみを固定し逸脱は fail-fast。`model=amazon.titan-embed-text-v2:0` / `dimensions=1024` / `region` は deploy 可変値の**既定**であり `assertPackageModeConstraints` では検証しない（Spec §5.3。運用者が GitHub Actions 変数で変更可）。
...
**Interfaces:**
- Produces: `EmbeddingConfig.provider` union に `"bedrock"`；`EmbeddingConfig.region?: string`／`EmbeddingConfig.profile?: string`；`Config.packageMode: boolean`。env `NEXUS_EMBEDDING_REGION` / `NEXUS_EMBEDDING_PROFILE` / `NEXUS_PACKAGE_MODE`。`export function assertPackageModeConstraints(config: { packageMode: boolean; embedding: Pick<EmbeddingConfig, 'provider'> }): void`（provider のみ検証する部分型を受け取り、テストからも完全な `EmbeddingConfig` を組まずに呼べる）。
...
describe('bedrock / packageMode config', () => {
  ...
  it('assertPackageModeConstraints passes for bedrock provider regardless of model/dimensions (deploy-variable)', () => {
    // packageMode locks provider only; model/dimensions/region stay deploy-variable (Spec §5.3).
    expect(() =>
      assertPackageModeConstraints({ packageMode: true, embedding: { provider: 'bedrock' } }),
    ).not.toThrow();
    expect(() =>
      assertPackageModeConstraints({ packageMode: false, embedding: { provider: 'ollama' } }),
    ).not.toThrow();
  });
});
```
...
/** パッケージモードでは埋め込み provider を bedrock に固定する（逸脱時は fail-fast）。model/dimensions/region は Spec §5.3 の deploy 可変値のためロックしない。 */
export const assertPackageModeConstraints = (
  config: { packageMode: boolean; embedding: Pick<EmbeddingConfig, 'provider'> },
): void => {
...
- [x] **Step 2: 実行権限を付与して staging を生成する**
...
- [x] **Step 3: 変換後 plugin.json とファイル集合を検証する**
...
- [x] **Step 4: ソースミラーが単体でビルドできることを検証する**
...
- [x] **Step 5: 変換後 plugin.json を検証する（claude plugin validate --strict）**

Run:
```bash
( cd /tmp/nexus-pkg-verify && npx claude plugin validate . --strict && echo "PLUGIN-VALIDATE-OK" )
```
Expected: `claude plugin validate --strict` 通過（変換後 plugin.json が有効）、最後に `PLUGIN-VALIDATE-OK`。Spec §6.5・§8（項目5）の要件を満たす。
...
- [x] **Step 1: Global Constraints を改訂する**
...
- [x] **Step 2: Task 1 の staging スクリプトを Plan A Task 4 に一本化する**

Plan B Task 1 の `scripts/stage-plugin-dist.sh` 実体（plugin.json を無変換 `cp` するコピー版）を撤去し、同一ファイルを二重定義しないため実装は Plan A Task 4 を唯一の正とする旨を明記する。

```markdown
- `scripts/stage-plugin-dist.sh` の実装は本リポジトリの実装計画 [2026-07-07-nexus-packaged-plugin-implementation.md](2026-07-07-nexus-packaged-plugin-implementation.md) Task 4 を唯一の正とする（plugin.json の stage 時変換つき: `userConfig` 除去、`mcpServers.nexus.env` を `NEXUS_PACKAGE_MODE=1` ＋ Bedrock 固定へ置換）。本計画では別実装を作らず、これを呼ぶだけとする。`packages/dashboard` は同梱維持（ローカル TUI のため）。
```

- [x] **Step 3: Prerequisites に AWS / GitHub Actions 変数を追記する**
...
- [x] **Step 5: Task 2 ワークフローの stage ステップに deploy 可変 env を注入する**（本 PR で Plan B に適用済み。deploy.md 288〜295 行目に反映されている内容を参照）

Plan B Task 2 の「Stage plugin source mirror」ステップ（`run: scripts/stage-plugin-dist.sh dist-staging`）に GitHub Actions 変数を env として渡す変更を明記する。これがないと deploy 可変値（Spec §5.3）が stage スクリプトに届かず、常に既定値が焼き込まれる。
...
- [x] **Step 6: markdownlint を確認してコミット**
...
- §3.1 LLM ロック（利用者は provider/model を選択不可 / model・dimensions・region は運用者が deploy 可変）→ Task 1（packageMode ＋ provider のみ検証する assertPackageModeConstraints）＋ Task 3（ハードロック配線）＋ Task 4（plugin.json 変換・userConfig 除去・固定 env 注入）。
...
- §5.1〜5.5 決定 → Task 1〜4 に反映（§5.3 deploy 可変・§5.5 provider のみ fail-fast に従い、`assertPackageModeConstraints` は provider のみ検証し model/dimensions/region はロックしない）。§6.1 API 形状 → Task 2 実装。§6.3 env サーフェス → Task 1。§6.4 エラー処理 → Task 2（`mapAwsError`・healthCheck）。§6.5 テスト（`claude plugin validate --strict`）→ Task 4 Step 5。§8 配布 → Task 4/5。ギャップなし。

**2. Placeholder scan**
- 「適切に」等の曖昧語・TODO・コード無し実装ステップは無し。全コードステップは実体コードを含む。

**3. Type/名称整合性**
- `assertPackageModeConstraints`（Task 1 定義 → Task 3 呼び出し）一致し、provider のみ検証する部分型引数（`{ packageMode: boolean; embedding: Pick<EmbeddingConfig, 'provider'> }`）のためテスト・factory 双方から `as any` なしで型整合する。`BedrockEmbeddingProvider`・`BedrockDependencies`（Task 2 定義 → Task 3 利用）一致。env 名（`NEXUS_EMBEDDING_REGION`/`_PROFILE`/`NEXUS_PACKAGE_MODE`）は Task 1・4・5 で一致。`EmbeddingConfig.region`/`profile`・`Config.packageMode`（Task 1 定義 → Task 2/3 利用）一致。
