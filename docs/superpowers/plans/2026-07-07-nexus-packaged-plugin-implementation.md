...
```
Expected: `claude plugin validate --strict` 通過（変換後 plugin.json が有効）、最後に `PLUGIN-VALIDATE-OK`。Spec §6.5/§8.5 の要件を満たす。
...
- [ ] **Step 5: Task 2 ワークフローの stage ステップに deploy 可変 env を注入する**
