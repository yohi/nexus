...
async search(queryVector: number[], topK: number, filter?: VectorFilter): Promise<VectorSearchResult[]> {
  return this.trackOp(() => {
    ...
    if (filter) {
      // VectorFilter オブジェクトを SQL WHERE 句文字列に変換する。
      // filePathPrefix → filePathPrefixFilter()（escapeLikeValue + ESCAPE '\' 付き LIKE 句）
      // language       → `language = '<escaped>'`
      // symbolKind     → `symbolKind = '<escaped>'`
      // 複数条件は AND で結合する。各値は validateFilterValue() → escapeFilterValue() を通す。
...
| ファイル | 影響 |
|----------|------|
| `tests/unit/indexer/pipeline.test.ts` | `compactAfterReindex()`・タイマー・`stop()` 動作等 8項目のテスト追加あり（「Pipeline テスト追加」セクション参照） |
| `tests/integration/pipeline.test.ts` | `LanceVectorStore` コンストラクタに `dbPath` を渡す修正のみ |
| `tests/unit/storage/compaction.test.ts` | そのまま維持 |
| `tests/unit/storage/in-memory-vector-store.ts` | `close()` の no-op 実装を追加（`IVectorStore` 準拠のため） |
