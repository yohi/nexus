...
`current` holds the most recent successful response for this route and is `null` after a poll failure or when no request has succeeded. On the first failure after success, `stale` captures that last successful `current` value. Further consecutive failures preserve the existing `stale` value; they must not replace it with `null`. `stale` is cleared only on port change or when the route recovers to `connected`. `lastSuccessAt` and `lastErrorAt` are Unix millisecond timestamps for the latest successful and the latest failed fetch respectively. `generation` is a monotonic integer incremented on every completed fetch attempt (success or failure) and reset on port change; it lets downstream history buffers detect missing polls even when the response payload is unchanged. `waiting` is the initial pending state; `unavailable` is emitted after the first failed retry so a discovered port whose runtime is dead can transition to `runtime_unavailable` instead of staying in `waiting` forever. Views render `current` only; Diagnostics renders `current` plus explicitly labeled `stale` values with their last-success timestamp.
...
For each counter and histogram component, the first sample establishes a baseline and has no rate. For consecutive samples, nondecreasing values contribute `current - previous`. A lower value indicates a reset: discard the old baseline, start at the lower value, and contribute **no** delta across that interval. Sum valid interval deltas when their ending sample is inside the five-minute window, even if the preceding baseline sample is before the window; keep that preceding sample solely to establish the baseline for the first in-window interval. A missing poll (detected by a `generation` gap) breaks consecutive intervals for every series; the next observation establishes a new baseline without a delta. A new series does not inherit another series' baseline. Derive request success/error and the telemetry Attention rule from these window deltas; never interpret a missing series as zero.
...
Unit tests cover snapshot-field sharing, health cache lifecycle, `ValueState` extraction, per-label history, `_sum`/`_count`/`_bucket` preservation, window deltas, reset handling, sparkline, readiness, and Attention provenance. Both polling hooks test `success -> failure -> failure` and prove `stale` remains the last successful value until recovery or port change. History tests use a bounded test window and timestamps to prove that expired samples are evicted while the immediately preceding sample remains as the delta baseline; port-change tests render old-port data/generation during the port-change commit and prove it is discarded before generation 1 from the new port is accepted. Ink component tests cover each view, resize and unavailable states, direct digits, Tab / Shift+Tab / Left / Right, ignored `h`/`l`, and `q`. The snapshot-builder regression test exercises a real `PluginRegistry`, `BedrockEmbeddingProvider`, and injected `BedrockClientLike.send` spy to prove that no provider probe occurs. HTTP integration tests cover GET-only, error JSON, loopback binding, and MCP call count; CLI integration tests cover startup, connection loss, polling independence, port replacement and cleanup of timers. Use fixtures with real response shapes and mock provider/network calls so tests cannot bill providers.
...
| `src/observability/dashboard-status-endpoint.ts` | **Reference for future implementation only:** GET-only dashboard `/status` helper using `buildDashboardIndexStatusSnapshot`. |
...
| `packages/dashboard/src/hooks/use-dashboard-status.ts`, `packages/dashboard/src/hooks/use-metrics-history.ts` | **References for future implementation only:** independent status polling and per-series five-minute telemetry history; keep both on the selected port. |
...
| Dashboard hooks, utilities, views, components, CLI, and their tests | All Create/Modify/Delete/Test paths listed in plan Tasks 5–9; the implementation plan File Map is the exhaustive path inventory. |

No additional new source-file paths are prescribed here. This review gate reconciles this design with the corresponding implementation plan and changes documentation only; production implementation remains blocked until the plan passes review. Future implementation must keep the status route internal, project-scoped, read-only and loopback-only; create no agent configuration, commit no local port files, credentials, or machine-specific absolute paths.
...
| --- | --- |
| RG-001: side-effect-free provider/status boundary, provider tri-state, MCP compatibility | Plan Tasks 1–4 use a shared non-provider collector, `buildDashboardIndexStatusSnapshot`, registry known-state cache and a builder-level no-probe/Bedrock regression test. |
